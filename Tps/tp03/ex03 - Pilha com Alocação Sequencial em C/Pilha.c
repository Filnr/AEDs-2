...
void inserir(Show show ,Show pilha[], int  *tam){
    // Adiciona o show a pilha, enquanto cresce o tam
    pilha[(*tam)++] = show;
}

Show remover(Show pilha[], int *tam){
    // retorna o show do tam, ja retirado da pilha
    return pilha[--(*tam)];
}
...
int main() {
    ...
    while(!ehFim(entrada)) {
        show_from_id(&pilha[tam], entrada);
        tam++;
        fgets(entrada, MAX_LINE, stdin);
        len = strlen(entrada);
        if(len > 0 && entrada[len-1]=='\n'){
    ...
    for(int i = 0; i < n; i++){
        ...
        if(strcmp(partes[0], "I") == 0){
            ...
            inserir(show, pilha, &tam);
        }
        else if(strcmp(partes[0], "R") == 0){
            removidos[ind++] = remover(pilha, &tam).title;
        }
    }
