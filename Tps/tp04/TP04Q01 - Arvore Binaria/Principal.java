...
class Arvore {
    ...
    public Arvore(Show show){
        this.raiz = new No(show);
        this.qtComps = 0;
    }
    ...
    private No inserir(No atual, Show show){
        if(atual == null){
            return new No(show);
        }
        // Se encontrar um NO vazio, insere o novo no
        // Procura um no vazio a depender da ordem alfabetica
        if(show.getTitle().compareTo(atual.getShow().getTitle()) < 0){
            atual.setEsq(inserir(atual.getEsq(), show));
        }
        else if(show.getTitle().compareTo(atual.getShow().getTitle()) > 0){
    ...
    private boolean pesquisar(No atual, String nome){
        ...
        if(atual != null){
            ...
            if(nome.compareTo(atual.getShow().getTitle()) < 0){
                System.out.printf(" esq");
                encontrado = pesquisar(atual.getEsq(), nome);
            }
            else if(nome.compareTo(atual.getShow().getTitle()) > 0){
                qtComps++;
