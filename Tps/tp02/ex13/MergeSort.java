...
public class MergeSort {
    ...
    private static int compararDuracaoTitulo(Show a, Show b) {
        // Função simplificada responsavel por realizar as comparações do array de shows
        int duracaoA = formataDuration(a);
        int duracaoB = formataDuration(b);
    
        int comparacao = Integer.compare(duracaoA, duracaoB);
        if (comparacao != 0) return comparacao;
        return a.getTitle().compareToIgnoreCase(b.getTitle());
    }
    

    public static int formataDuration(Show show){
        // Pega a parte numeria da duração e transforma em int para comparações
        String duration = show.getDuration();
        int resultado = 0;
        if(duration != null && !duration.equals("NaN")){
