...
class Show {
    ...
    public int comparar(Show outro) {
        // Função responsavel por comparar os diretores
        int comparacaoDiretor = this.director.compareToIgnoreCase(outro.director);
        if (comparacaoDiretor != 0) {
            // Se os diretores forem diferentes, retorna esse valor
            return comparacaoDiretor;
        } else {
            // Se os diretores forem iguais, utiliza o title para desempatar
...
public class HeapSort {
    ...
    public static void heapify(Show[] shows, int n, int i, int[] movi, int[] comp) {
        // Inicializa os valores index do heap
        int maior = i;
        int esq = 2 * i + 1;
        int dir = 2 * i + 2;
        
        // Verifica se o filho da esquerda é maior que a da raiz
        if (esq < n) {
            comp[0]++;
            if (shows[esq].comparar(shows[maior]) > 0) {
                maior = esq;
            }
        }

        // verifica o da direita com a raiz
        if (dir < n) {
            comp[0]++;
            if (shows[dir].comparar(shows[maior]) > 0) {
                maior = dir;
            }
        }
    
        // Se o maior elemento não for a raiz, troca com a raiz
        if (maior != i) {
            Show temp = shows[i];
            shows[i] = shows[maior];
            shows[maior] = temp;
            ...
            heapify(shows, n, maior, movi, comp);
        }
    }
    
    public static void heap(Show[] shows, int tam, int[] movi, int[] comp) {
        // Construir o heap (reorganiza o array)
        for (int i = tam / 2 - 1; i >= 0; i--) {
            heapify(shows, tam, i, movi, comp);
        }

        // Um por um, retira o maior elemento e coloca na posição correta
        for (int i = tam - 1; i >= 0; i--) {
            ...
            // Chama o heapify no restante do heap
            heapify(shows, i, 0, movi, comp);
        }
    }
