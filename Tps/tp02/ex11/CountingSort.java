...
public class CountingSort {
    ...
    public static int buscaValorMax(Show shows[], int tam) {
        // Função que percorre o array de shows em busca do maior ano de lançamento
        int maior = shows[0].getRelease_year();
        for (int i = 1; i < tam; i++) {
            if (shows[i].getRelease_year() > maior) {
                maior = shows[i].getRelease_year();
            }
        }
        return maior;
    }

    public static int buscaValorMin(Show shows[], int tam) {
        // Função que percorre o array de shows em busca do menor ano de lançamento
        int menor = shows[0].getRelease_year();
        for (int i = 1; i < tam; i++) {
            if (shows[i].getRelease_year() < menor) {
                menor = shows[i].getRelease_year();
            }
        }
        return menor;
    }
    ...
    public static void ordenaInser(Show shows[], int tam, int[] movi, int[] comp) {
        ...
        for (int i = 1; i < tam; i++) {
            ...
            while (j >= 0 && shows[j].getRelease_year() == menor.getRelease_year() &&
                    shows[j].getTitle().compareToIgnoreCase(menor.getTitle()) > 0) {
                movi[0]++;
                comp[0]++;
                shows[j + 1] = shows[j];
                j--;
    ...
    public static void ordenarCounting(Show shows[], int tam, int[] comp, int[] mov) {
        // Função Counting responsavel por ordenar os shows em ordem de ano de lançamento
        // Busca o maior valor de ano de lançamento para criar um array que contenha todos os numeros
        int maiorAno = buscaValorMax(shows, tam);
        int menorAno = buscaValorMin(shows, tam);
        int[] contagem = new int[maiorAno - menorAno + 1];
        for (int i = 0; i < tam; i++) {
            contagem[shows[i].getRelease_year() - menorAno]++;
            mov[0]++;
        }
        // Formatando o array para ordenar o vetor de saida
        for (int v = 1; v < contagem.length; v++) {
            contagem[v] += contagem[v - 1];
            mov[0]++;
            comp[0]++;
        }
        // preparando o vetor ordenado
        Show[] saida = new Show[tam];
        for (int i = tam - 1; i >= 0; i--) {
            int valor = shows[i].getRelease_year() - menorAno;
            int posicao = contagem[valor] - 1;
            saida[posicao] = shows[i];
            contagem[valor]--;
            mov[0] += 2;
            comp[0]++;
        }
        // Ordenação do array de shows
        for (int i = 0; i < tam; i++) {
            mov[0]++;
            comp[0]++;
            shows[i] = saida[i];
        }
        ordenaInser(shows, tam, comp, mov);
    }
    ...
    public static void main(String[] args) {
        ...
        int movi[] = { 0 };
        int comp[] = { 0 };
        ordenarCounting(shows, tam, movi, comp);
        for (int i = 0; i < tam; i++) {
            shows[i].imprimir();
        }
        ler.close();
